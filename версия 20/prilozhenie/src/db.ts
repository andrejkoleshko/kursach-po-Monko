...
// 🔥 Добавляем колонку reaction, если её нет
try {
  db.exec("ALTER TABLE assistant_history ADD COLUMN reaction TEXT");
} catch {
...
export function addAssistantEntry(userId: number, question: string, answer: string, usedDocs: string[]): AssistantEntry {
  const stmt = db.prepare(`
    INSERT INTO assistant_history (user_id, question, answer, used_docs, created_at)
    VALUES (@user_id, @question, @answer, @used_docs, @created_at)
  `);
  ...
  const info = stmt.run({
    ...
    question,
    answer,
    used_docs: usedDocs.join(', '),
    created_at
  });
  return { id: Number(info.lastInsertRowid), user_id: userId, question, answer, used_docs: usedDocs.join(', '), created_at };
}

export function listAssistantHistory(userId: number): AssistantEntry[] {
  return db.prepare(`SELECT * FROM assistant_history WHERE user_id = ? ORDER BY datetime(created_at) ASC`).all(userId) as AssistantEntry[];
}

export function toggleReaction(entryId: number, reaction: string): string[] {
  // читаем текущие реакции
  const row = db.prepare(`
    SELECT reaction FROM assistant_history WHERE id = ?
  `).get(entryId) as { reaction: string | null };

  let list: string[] = [];

  if (row?.reaction) {
    try {
      list = JSON.parse(row.reaction);
    } catch {
      list = [];
    }
  }

  // если реакция уже есть — убираем
  if (list.includes(reaction)) {
    list = list.filter(r => r !== reaction);
  } else {
    // иначе добавляем
    list.push(reaction);
  }
  ...
  db.prepare(`
    ...
    SET reaction = ?
    WHERE id = ?
  `).run(JSON.stringify(list), entryId);
