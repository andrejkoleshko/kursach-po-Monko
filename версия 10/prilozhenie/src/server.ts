...
// 🔥 Админ‑маршруты для лимита
app.get('/api/admin/upload-limit', (req: Request, res: Response) => {
  try {
...
app.patch('/api/admin/upload-limit', (req: Request, res: Response) => {
  try {
    let { upload_limit_mb } = req.body;
    const value = Number(upload_limit_mb); // 🔥 приводим к числу

    if (isNaN(value) || value <= 0) {
      return res.status(400).json({ error: 'Некорректное значение лимита' });
    }

    db.setUploadLimit(value); // сохраняем как число (в базе всё равно будет строка)
...
app.post('/api/upload', auth.authRequired, (req: Request, res: Response) => {
  try {
    ...
    busboy.on('file', (fieldname: string, file: NodeJS.ReadableStream, info: any) => {
      ...
      const MAX_SIZE = db.getUploadLimit() * 1024 * 1024; // 🔥 лимит из базы
      console.log("Текущий лимит:", db.getUploadLimit());
