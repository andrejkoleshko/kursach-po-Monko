...
// Загружаем профиль пользователя, чтобы взять avatar_url
...
// Функция плавной автопрокрутки вниз
...
async function sendQuestion() {
  ...
  // 🔥 сразу рисуем сообщение пользователя
  ...
  userMsg.innerHTML = `
    <div class="me-2 text-end">
      ...
      <div class="message-meta">
        ${timeNow} <span class="status delivered">✔</span>
      </div>
    </div>
  ...
  // статус меняется через 2 секунды
  setTimeout(() => {
...
// Автоматическая загрузка при смене категории
...
async function loadDocuments(query = '', category = 'all') {
  ...
  list.innerHTML = docs.map(d => `
    <div class="form-check">
      ...
      <label class="form-check-label" for="doc-${d.id}">
        ${d.original_name}
      </label>
    </div>
  `).join('');
...
async function loadHistory() {
  ...
  history.forEach(h => {
    ...
    assistantMsg.innerHTML = `
      <img src="/images/assistant.png" class="avatar me-2">
      <div>
        <div class="bubble-assistant">${h.answer}</div>
