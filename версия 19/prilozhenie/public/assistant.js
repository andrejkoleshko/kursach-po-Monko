...
async function sendQuestion() {
  ...
  const bubbleIdUser = `bubble-${Date.now()}-user`;
  ...
  userMsg.innerHTML = `
    <div class="me-2 text-end">
      <div id="${bubbleIdUser}" class="bubble-user">${question}</div>

      <div class="reaction-button" onclick="openReactionsMenu('${bubbleIdUser}', event)">➕</div>
      <div class="message-reactions" id="react-${bubbleIdUser}"></div>
  ...
  assistantMsg.innerHTML = `
    ...
    <div>
      ...
      <div class="message-reactions" id="react-${bubbleId}"></div>
  ...
  history.forEach(h => {
    ...
    userMsg.innerHTML = `
      <div class="me-2 text-end ${hideUserAvatar ? 'mt-0' : ''}">
        <div id="${bubbleId}-user" class="bubble-user">${h.question}</div>
...
// ГЛОБАЛЬНАЯ функция — теперь доступна везде
function setReaction(bubbleId, reaction) {
  const container = document.getElementById(`react-${bubbleId}`);
  if (container) container.textContent = reaction;
...
function openReactionsMenu(bubbleId, event) {
  console.log("OPEN MENU:", bubbleId);
  ...
  reactions.forEach(r => {
    ...
    span.onclick = (e) => {
      e.stopPropagation();
      setReaction(bubbleId, r);
    };
    reactionMenuEl.appendChild(span);
  });

  bubble.parentElement.appendChild(reactionMenuEl);
