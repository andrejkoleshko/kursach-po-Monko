...
app.delete('/api/admin/users/:username',
  ...
  (req: Request, res: Response) => {
    try {
      ...
      // 🔥 Нельзя удалять равных или выше
...
app.patch('/api/admin/users/:id/role',
  ...
  (req: Request, res: Response) => {
    ...
    // 🔥 Нельзя менять роль равных или выше
    if (ROLE_LEVEL[current.role] <= ROLE_LEVEL[target.role]) {
      return res.status(403).json({ error: 'Недостаточно прав для изменения роли этого пользователя' });
    }
